...
# class GetAngle(nn.Module):
#     def __init__(self, fingerprint_dim):
#         super(GetAngle, self).__init__()
#         self.output = nn.Linear(fingerprint_dim, 1)

#     def forward(self, mol_feature, reverse=False):
# #         print(mol_feature.shape,mol_feature)
#         output = self.output(mol_feature)
#         temp_one = torch.ones_like(mol_feature)
# #         print(output.shape,output)        
#         output = abs( temp_one*output/((temp_one*temp_one*output*output)**0.5) )
# #         print(output.shape,output)
#         return output
...
#         self.output = nn.Parameter(torch.rand(feature_num, class_num), requires_grad=True)
    ...
    def forward(self, atom_list=None, bond_list=None, atom_degree_list=None, bond_degree_list=None, atom_mask=None, output_feature=False, feature_only=False, feature=None,arc=False,s=6,m=0.5):
        if feature_only:
            mol_prediction = self.output(self.dropout(feature))
...
#             print(mol_context.shape,mol_context)
        ...
        if arc:
            x = nn.functional.normalize(mol_feature, dim=1)
            w = (nn.functional.normalize(self.output.weight, dim=1)).t()
#             w = -s * w #与-W的夹角
#             print(s)
...
#             print(w.shape)——torch.Size([1, 200])

#         print(W.shape)
        if output_feature:
#             print(mol_prediction.shape)——torch.Size([10, 1])
#             print(mol_feature.shape)——torch.Size([10, 200])
