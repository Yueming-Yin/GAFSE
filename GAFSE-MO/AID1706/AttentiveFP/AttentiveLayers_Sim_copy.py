...
class AFSE(nn.Module):
    ...
    def forward(self,feature=None, d=0, output_lr=False, sigmoid=False):
        ...
        mol_prediction = self.output(self.dropout(self.BN(self.metric(self.dropout(torch.cat((feature, feature+d),-1))))))
        if sigmoid:
...
#         beta_MSE = torch.norm(2*(aW1**2+aW2**2))**2
#         conv_lr = 1/(beta_AFSE+beta_MSE+1e-9)
#         mol_prediction = self.output(self.dropout(self.BN(self.hidden(self.dropout(self.BN(self.metric(self.dropout(torch.cat((feature, feature+d),-1)))))))))
#         if sigmoid:
#             mol_prediction = torch.sigmoid(mol_prediction)
#         if output_lr:
#             return mol_prediction, conv_lr
...
#         self.fc_share = nn.Linear(2*fingerprint_dim, fingerprint_dim)
#         self.fc = nn.ModuleList([nn.Linear(fingerprint_dim, int(fingerprint_dim/2)) for r in range(output_units_num)])
#         self.output = nn.ModuleList([nn.Linear(int(fingerprint_dim/2), 1) for r in range(output_units_num)])
#         self.fingerprint_dim = fingerprint_dim
...
#             aW1 = self.output[i](self.dropout(self.fc[i](self.dropout(self.fc_share(self.dropout(torch.cat((unit_vector, zero_vector),-1)))))))
#             aW2 = self.output[i](self.dropout(self.fc[i](self.dropout(self.fc_share(self.dropout(torch.cat((zero_vector, unit_vector),-1)))))))
#             beta_AFSE = torch.norm(24*aW2**2/(aW1+aW2))**2
#             beta_MSE = torch.norm(2*(aW1**2+aW2**2))**2
#             conv_lr += 1/(beta_AFSE+beta_MSE+1e-9)
#             conv_lr_list.append(conv_lr.detach())
#             task_mol_prediction = self.output[i](self.dropout(self.fc[i](self.dropout(self.fc_share(self.dropout(torch.cat((feature, feature+d),-1))))))) #self.attend[i](torch.cat((feature, feature+d),-1))*
#             mol_prediction = torch.cat([mol_prediction, task_mol_prediction], -1)
#         conv_lr_average = conv_lr / self.output_units_num
#         if sigmoid:
#             mol_prediction = torch.sigmoid(mol_prediction)
#         if output_lr:
#             return mol_prediction, conv_lr_average
#         return mol_prediction
